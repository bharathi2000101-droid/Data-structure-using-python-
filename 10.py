class JobScheduler:
    ...
    def _heapify_down(self, index):
        ...
        while True:
            ...
            if (
                left_child < size
                and self.heap[left_child][0] > self.heap[largest][0]
            ):
                largest = left_child

            if (
                right_child < size
                and self.heap[right_child][0] > self.heap[largest][0]
            ):
                largest = right_child
    ...
    def delete_max_job(self):
        """b) Delete the Highest Priority Job (Extract Max)"""
        if not self.heap:
            print("\nHeap is empty!")
            return
    ...
    def peek_max_job(self):
        """c) Peek at the Job with the Highest Priority"""
        if not self.heap:
            print("\nHeap is empty!")
            return
        max_job = self.heap[0]
